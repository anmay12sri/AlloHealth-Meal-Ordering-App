    ...
    <div className="py-4 flex flex-col md:flex-row duration-300 px-4 border-b border-yellow-300 bg-white shadow-sm rounded-lg hover:shadow-lg transition-shadow">
      <div className="flex flex-col md:flex-row w-full md:justify-between">
        ...
        <div className="flex flex-col md:ml-6 w-full md:w-auto">
          <h3 className="text-lg md:text-xl font-bold text-indigo-600">{meal?.title}</h3>
            ...
            {meal.drinks?.map((drink) => (
              <div key={drink?.id} className="flex flex-col items-center">
                <img
                  src={drink?.img}
                  alt={drink?.title}
                  className="w-12 h-12 md:w-16 md:h-16 rounded-lg border-2 border-transparent hover:border-indigo-500 transition-colors cursor-pointer"
